...
#define MIXER_PLAYBACK_PAMUTE_SWITCH   		"Playback PAMUTE SWITCH"
...
struct route_setting line_in_routing[] = {
	{
		.ctl_name = MIXER_LINER_SWITCH,
		.intval = 1,
	},
	{
...
struct mixer_ctls
{
	...
	struct mixer_ctl *fmr_switch;					// do not use tmp
...
struct sunxi_audio_device {
    ...
    bool bluetooth_nrec;
    int wb_amr;
	int pa_handle;		// star add for pa control
...
static void tinymix_list_controls(struct mixer *mixer)
{
    ...
    LOGD("Number of controls: %d", num_ctls);

    LOGD("ctl\ttype\tnum\t%-40s value", "name");
    for (i = 0; i < num_ctls; i++) {
        ...
        num_values = mixer_ctl_get_num_values(ctl);
        LOGD("%d\t%s\t%d\t%-40s", i, type, num_values, buffer);
...
{
	LOGV("RequestWriteBuf ++: size: %d", inSize);

	if (PcmManager->BufValideLen< inSize)
	{
		LOGE("not enough buffer to write");
		return -1;
	}
	...
	LOGV("after wr: BufTotalLen: %d, DataLen: %d, BufValideLen: %d, BufReadPtr: %p, BufWritPtr: %p", 
...
static int ReadPcmData(void *pBuf, int uGetLen, struct pcm_buf_manager *PcmManager)
{
	...
	int timeout, max_wait_count;

	max_wait_count = uGetLen * 100 / (PcmManager->SampleRate*PcmManager->Channel * 2) + 1; //normal
	max_wait_count *= 2;//twice

	LOGV("ReadPcmDataForEnc ++, getLen: %d max_wait_count=%d", uGetLen,max_wait_count);
    while(PcmManager->DataLen < uGetLen)
    {
        LOGV("pcm is not enough for audio encoder! uGetLen: %d, uDataLen: %d\n",
        ...
        if(timeout > max_wait_count) {
        	if (PcmManager->DataLen < uGetLen) {
        		...
        		LOGW("fill with dc size:%d",uGetLen - PcmManager->DataLen);
        	}
        	break;
        }
    }
	...
	LOGV("after rd: BufTotalLen: %d, DataLen: %d, BufValideLen: %d, pBufReadPtr: %p, pBufWritPtr: %p", 
		PcmManager->BufTotalLen, PcmManager->DataLen, PcmManager->BufValideLen, 
		PcmManager->BufReadPtr, PcmManager->BufWritPtr);
...
static int find_name_map(struct sunxi_audio_device *adev, char * in, char * out)
{
	...
	for (; index < AUDIO_MAP_CNT; index++)
	{
		if (strlen(audio_name_map[index].name_linux) == 0)
		{
			...
			LOGD("linux name = %s, android name = %s",
...
static void init_audio_devices(struct sunxi_audio_device *adev)
{
	...
	for (card = 0; card < MAX_AUDIO_DEVICES; card++)
	{
		...
		{
			// break;
			LOGV("card: %d, name: %s, capture: %d, playback: %d",
...
static void init_audio_devices_active(struct sunxi_audio_device *adev)
{
	...
	if (ret > 0)
	{
		LOGV("init_audio_devices_active: get property %s: %s", PRO_AUDIO_INPUT_ACTIVE, prop_value_in);
	...
	{
		LOGV("init_audio_devices_active: get property %s failed, %s", PRO_AUDIO_INPUT_ACTIVE, strerror(errno));
	}
	...
	if (ret > 0)
	{
		LOGV("init_audio_devices_active: get property %s: %s", PRO_AUDIO_OUTPUT_ACTIVE, prop_value_out);
	...
	{
		LOGV("init_audio_devices_active: get property %s failed, %s", PRO_AUDIO_OUTPUT_ACTIVE, strerror(errno));
	}
	...
	for (card = 0; card < MAX_AUDIO_DEVICES; card++)
	{
			...
			&& (!strcmp(adev->dev_manager[card].name, AUDIO_NAME_CODEC)))
		{
			if (!(flag_active & AUDIO_IN)
				&& (adev->dev_manager[card].flag_in == AUDIO_IN))
			{
				LOGV("OK, default use %s capture", adev->dev_manager[card].name);
			...
			if (!(flag_active & AUDIO_OUT)
				&& (adev->dev_manager[card].flag_out == AUDIO_OUT))
			{
				LOGV("OK, default use %s playback", adev->dev_manager[card].name);
...
INIT_END:
	...
	{
		LOGW("there is not a audio capture devices");
	}
	...
	{
		LOGW("there is not a audio playback devices");
	}

	LOGV("OK, default adev->devices: %08x", adev->devices);
}
...
static char * get_audio_devices(struct sunxi_audio_device *adev, int in_out)
{
	...
	for(card = 0; card < MAX_AUDIO_DEVICES; card++)
	{
		if (adev->dev_manager[card].flag_exist == true)
		{
			if (adev->dev_manager[card].flag_in == AUDIO_IN)
			{
				...
				strcat(in_devices, ",");
				LOGD("in dev:%s",adev->dev_manager[card].name);
			}
			...
			if (adev->dev_manager[card].flag_out == AUDIO_OUT)
			{
				...
				strcat(out_devices, ",");
				LOGD("out dev:%s",adev->dev_manager[card].name);
			}
		}
	}
	...
	if (in_out & AUDIO_IN)
	{
		LOGD("in capture: %s",in_devices);
		return in_devices;
	}
	else if(in_out & AUDIO_OUT)
	{
		LOGD("out playback: %s",out_devices);
		return out_devices;
	}
	else
	{
		LOGE("unknown in/out flag");
		return 0;
	}
}
...
static int set_audio_devices_active_internal(struct sunxi_stream_out *stream, int in_out, int value)
{
	...
	if (in_out & AUDIO_OUT)
	{
			...
			/*&& (strlen(adev->out_device_active_req) > 0)*/)
		{
			...
			LOGV("get property %s: %s", PRO_AUDIO_OUTPUT_ACTIVE, prop_value_out);
			if (!strstr(prop_value_out, devices))
			{
				strcat(devices, ",");
...
static int set_audio_devices_active(struct sunxi_audio_device *adev, int in_out, char * devices)
{
	...
	if (in_out & AUDIO_IN)
	{
		...
		if (ret < 0)
		{
			LOGE("set property %s: %s failed", PRO_AUDIO_INPUT_ACTIVE, devices);
		}
		else
		{
			LOGV("set property %s: %s ok", PRO_AUDIO_INPUT_ACTIVE, devices);
		}
	}
	...
	if (in_out & AUDIO_OUT)
	{
		...
		if (ret < 0)
		{
			LOGE("set property %s: %s failed", PRO_AUDIO_OUTPUT_ACTIVE, devices);
		}
		else
		{
			LOGV("set property %s: %s ok", PRO_AUDIO_OUTPUT_ACTIVE, devices);
		}
	}
	...
	for (i = 0; i < cnt; i++)
	{
		for (card = 0; card < MAX_AUDIO_DEVICES; card++)
		{
			if (in_out & AUDIO_IN)
			{
				...
				{
					LOGV("%s %s device will be active", name[i], "input");
					adev->dev_manager[card].flag_in_active = 1;
			...
			else
			{
				if ((adev->dev_manager[card].flag_out == in_out)
					&& (strcmp(adev->dev_manager[card].name, name[i]) == 0))
				{
					LOGV("%s %s device will be active", name[i], "output");
					adev->dev_manager[card].flag_out_active = 1;
					break;
				}
			}
		...
		{
			LOGE("can not set %s %s active", name[i], (in_out & AUDIO_IN) ? "input" : "ouput");
			return -1;
		}
	}
...
static int get_audio_devices_active(struct sunxi_audio_device *adev, int in_out, char * devices)
{
	...
	LOGD("get_audio_devices_active: %s", devices);
...
static void select_mode(struct sunxi_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        LOGV("Entering IN_CALL state, in_call=%d", adev->in_call);
        if (!adev->in_call) {
        ...
        LOGV("Leaving IN_CALL state, in_call=%d, mode=%d",
...
static void select_output_device(struct sunxi_audio_device *adev)
{
	...
	LOGV("select_output_device, devices: %x, mode: %x", adev->devices, adev->mode);
	...
	if (ret > 0)
	{
		LOGD("get property audio.without.earpiece: %s", prop_value);
	...
	// mute/unmute speaker
	ret = ioctl(adev->pa_handle, pa_should_on ? PA_OPEN : PA_CLOSE, 0);	// set pa on/off
	if (ret < 0)
	{
		LOGE("set pa on/off failed");
	}
...
static int start_output_stream(struct sunxi_stream_out *out)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL)
	{
		LOGW("mode in call, do not start stream");
		return 0;
	}
	...
	for (index = 0; index < MAX_AUDIO_DEVICES; index++)
	{
		...
		{
			card = index;
			LOGV("use %s to playback audio", adev->dev_manager[index].name);
			...
			if (!pcm_is_ready(out->multi_pcm[card])) {
        		LOGE("cannot open pcm driver: %s", pcm_get_error(out->multi_pcm[card]));
			...
			if (DEFAULT_OUT_SAMPLING_RATE != out->multi_config[card].rate)
			{
				...
				if (ret != 0)
				{
					LOGE("create out resampler failed, %d -> %d", DEFAULT_OUT_SAMPLING_RATE, out->multi_config[card].rate);
					return ret;
				}

				LOGV("create out resampler OK, %d -> %d", DEFAULT_OUT_SAMPLING_RATE, out->multi_config[card].rate);
			}
			else
			{
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
	...
	LOGV("out_set_parameters: %s", kvpairs);
    ...
    if (ret >= 0) {
        ...
        if (((adev->devices & AUDIO_DEVICE_OUT_ALL) != val) && (val != 0)) {
			...
			LOGD("val: %x, adev->devices: %x", val, adev->devices);
	...
	if (ret >= 0)
	{
		LOGV("out AUDIO_PARAMETER_DEVICES_OUT_ACTIVE: %s", value);
		...
		{
			LOGW("in raw mode, should not set other audio out devices");
			return -1;
		}
	...
	if (ret >= 0)
	{
		...
		LOGD("AUDIO_PARAMETER_RAW_DATA_OUT: %d", bval);
		pthread_mutex_lock(&adev->lock);
		pthread_mutex_lock(&out->lock);
                         ...
                         size_t bytes)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL)
	{
		// LOGW("mode in call, do not out_write");
		return 0;
	}
	...
	if (adev->wifidiaplay_flag && adev->PcmManager.BufExist) {
...
static int start_input_stream(struct sunxi_stream_in *in)
{
	...
	if (adev->active_output)
	{
		int out_rate = adev->active_output->config.rate;

		if (!(out_rate % SAMPLING_RATE_8K))
		{
			// out stream is 48K serial
		...
		if (!(out_rate % SAMPLING_RATE_11K))
		{
			// out stream is 44.1K serial
		...
		LOGV("there is an active output stream, rate: %d, force capture rate: %d", out_rate, in_ajust_rate);
	}
	...
	for (card = 0; card < MAX_AUDIO_DEVICES; card++)
	{
		...
		{
			LOGV("use %s to capture audio", adev->dev_manager[card].name);
			break;
		}
	}
	...
	if (in->requested_rate != in->config.rate) {
		...
		ret = create_resampler(in->config.rate,
							   ...
							   &in->resampler);
		if (ret != 0) {
			LOGE("create in resampler failed, %d -> %d", in->config.rate, in->requested_rate);
			ret = -EINVAL;
			goto err;
		}

		LOGV("create in resampler OK, %d -> %d", in->config.rate, in->requested_rate);
	}
	else
	{
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
	...
	LOGV("in_set_parameters: %s", kvpairs);
	...
	if (ret >= 0)
	{
		LOGV("in_set_parament AUDIO_PARAMETER_DEVICES_IN_ACTIVE: %s", value);
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
	...
	if (ret >= 0)
	{
		LOGV("in AUDIO_PARAMETER_DEVICES_IN_ACTIVE: %s", value);
		set_audio_devices_active(adev, AUDIO_IN, value);
	}
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL)
	{
		...
		LOGV("adev_set_voice_volume, volume: %f, vol: %d", volume, vol);
                                  ...
                                  struct audio_stream_in **stream_in)
{
	...
	ladev->wifidiaplay_flag = false;
	if (devices == AUDIO_DEVICE_IN_WIFI_DISPLAY) {
		LOGV("to malloc PcmManagerBuffer: Buffer_size: %d", WD_BUFFER_SIZE);
    	ladev->PcmManager.BufStart= (unsigned char *)malloc(WD_BUFFER_SIZE);
		...
		ladev->PcmManager.BufTotalLen = WD_BUFFER_SIZE;
		...
		ladev->PcmManager.SampleRate = *sample_rate;
		ladev->PcmManager.Channel = 2;
		
		ladev->wifidiaplay_flag = true;
	}
                     ...
                     hw_device_t** device)
{
	...
	// pa control
	adev->pa_handle = open("/dev/pa_dev", O_RDONLY);
	if (adev->pa_handle <= 0)
	{
		free(adev);
		LOGE("Unable to open PA controller, aborting.");
        return -EINVAL;
	}

	ret = ioctl(adev->pa_handle, PA_OPEN, 0);
	if (ret < 0)
	{
		LOGE("set pa on failed");
	}

	adev->raw_flag = false;
	adev->wifidiaplay_flag = false;
	...
	for (card = 0; card < MAX_AUDIO_DEVICES; card++)
	{
		if (adev->dev_manager[card].flag_exist
			...
			&& !strcmp(adev->dev_manager[card].name, AUDIO_NAME_CODEC))
		{
			LOGV("use %s mixer control", adev->dev_manager[card].name);
			break;
		}
	}
	...
	if (card == MAX_AUDIO_DEVICES)
	{
		LOGE("can not find audio codec mixer control");
	}
	...
	if (ret > 0)
	{
		LOGV("get property %s: %s", PRO_AUDIO_MULTI_OUTPUT, prop_value);
		if (strcmp(prop_value, "true") == 0)
		{
...
//	adev->support_multi_ouput = true;		//for test
