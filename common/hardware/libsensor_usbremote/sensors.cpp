...
//#define SENSOR_NAME			"USBRemote_Sensor"
#define SENSOR_NAME				"USBRemote_Sensor_1915_af11"
...
#ifdef DEBUG_SENSOR
			LOGD("Sensor data: t x,y,z: %f %f, %f, %f\n",
...
static int sensor_get_class_path(sensors_poll_context_t *dev)
{
	...
	while((de = readdir(dir)))
	{
		...
		LOGD("buf(%s, %d), SENSOR_NAME(%s, %d)\n", buf, sizeof(buf), SENSOR_NAME, sizeof(SENSOR_NAME));

		if (strcmp(buf, SENSOR_NAME) == 0)
//		if (strncmp(buf, SENSOR_NAME, sizeof(SENSOR_NAME)) == 0)
		{
		    found = 1;
		    close(fd);
		    break;
...
static int open_input_device(void)
{
	...
	*filename++ = '/';
	...
	while ((de = readdir(dir)))
	{
		...
		LOGD("name(%s, %d), SENSOR_NAME(%s, %d)\n", name, sizeof(name), SENSOR_NAME, sizeof(SENSOR_NAME));

		if (!strcmp(name, SENSOR_NAME))
//		if (!strncmp(name, SENSOR_NAME, sizeof(SENSOR_NAME)))
		{
#ifdef DEBUG_SENSOR
		LOGD("devname is %s \n", devname);
#endif
...
static const struct sensor_t sSensorList[] = {
        ...
        { 	"usbremote 3-axis Accelerometer",
                "Bosch",
                1, 0,
